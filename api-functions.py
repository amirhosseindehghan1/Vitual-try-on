...
def segment_function(opt, up, gauss, seg, img_agnostic, parse_agnostic, pose, c, cm):
    ...
    for j in range(len(labels)):
        ...
        print("========== finished segmenation ========== ")
    return parse

def gmm_function(img_agnostic, parse, pose, c, cm, gmm):
...
def test(opt, seg, gmm, alias):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            # for img_name, c_name in zip(img_names, c_names):
            #     unpaired_names.append('{}_{}'.format(str(random.getrandbits(128)), c_name))
            # image_names.append(unpaired_names[0]) 
        ...
        print("image_namesssssssssssss", image_names)
...
def test_api(data: Dict[Any, Any] = None):
    ...
    try:
        response = requests.post("http://192.168.29.242:8000/virtual_try_on", json=postData)
...
async def tooba_try_on(data: Dict[Any, Any] = None):
    ...
    print(result_list)
    return JSONResponse(content=jsonable_encoder(result_list[0]))
